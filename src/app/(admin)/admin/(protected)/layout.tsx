...
import { AdminHeader, type AdminNavItem } from "./admin-header";
...
export default async function ProtectedAdminLayout({
  ...
  const user = await db.user.findUnique({
    ...
    select: { name: true, role: true, isActive: true, mustChangePassword: true },
  });
  ...
  const navItems: AdminNavItem[] = [
    ...
    // Inquiries hold personal data — EDITOR can't see them (same rule as
    // Application, CLAUDE.md §5), so don't even link there. Users management
    // is ADMIN-only too; the route and every action enforce it (CLAUDE.md
    // §7), this just hides the links.
    ...(isAdmin
      ? [
  ...
  return (
    <div className="flex min-h-screen flex-col">
      <AdminHeader userName={user.name} userRole={user.role} navItems={navItems} />
